...
## Changelog
- 2026-08-25 — Bookmark import completed: the remaining 157 of 326 classified links written (first 169 were written in an earlier run this same day). 325 of 326 kept links are now in the vault; the 1 unwritten row (`svk.se/aktorsportalen/.../handel-prissattning`, mapped to `Electricity_Markets_and_Prices`) was skipped as a genuine duplicate of an existing, more detailed single-note bullet already on that page. New pages written: `Data_Visualization`, `Noise_Modelling`, `Agentic_Coding` (all three were already named rows in this file, seeded from the original topic list / bookmarks; this is their first content). New candidates: Hobby Projects (3 notes, on `Unsorted`) and Hybrid Plants and Power-to-X (1 note, on `Energy_Transition_and_Scenarios`) — both already anticipated in the *Candidate topics* list below. `mkdocs.yml` nav and `notes/index.md` updated to match.
...
- 2026-08-25 — Zotero bulk import completed. New pages: `R`, `Greenhouse_Horticulture`, `Statistics_and_Data_Analysis`, `Vertical_Farming_and_Urban_Agriculture`, `Forestry_and_Agriculture_Statistics`, `Global_Datasets_and_Catalogs`, `Cartography_and_Map_Design`, `Energy_Market_and_Grid_Data`, `Optimization_and_Decision_Making`, `Reporting_and_Publishing`, plus the two species hubs `Norway_Spruce` and `Scots_Pine` that the map already declared. New candidates: Edible Insects and Medicinal Plants (on `Agriculture`), Origin of Life Photochemistry (on `Photosynthesis_and_Gas_Exchange`), UV Radiation and Vitamin D (on `Solar_Radiation_Modelling`), Mediterranean Forest History (on `Unsorted`). New tag values: `#project/msc-thesis`, `#project/own-publications`, `#crop/basil`, `#crop/wheat`, `#region/mexico`, `#region/spain`, `#region/germany`, `#region/finland`, `#region/netherlands`. Classification is content-first: the Zotero collection is only a prior, overridden whenever the title and abstract point elsewhere.
